...
class OnlineUser
{
    ...
    public function handle(Request $request, Closure $next)
    {
        if (Auth::check()) {
            ...
            if (empty($users)) {
                Cache::put('online-users', [['id' => Auth::user()->id, 'role' => Auth::user()->role, 'last_activity_at' => now()]], now()->addMinutes(10));
                return $next($request);
            }
            ...
            $users[] = ['id' => Auth::user()->id, 'role' => Auth::user()->role, 'last_activity_at' => now()];
